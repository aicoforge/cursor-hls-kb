...
連線後可瀏覽資料庫的 5 張資料表。完成安裝時僅 `hls_rules` 已由 `import_rules.py` 匯入規則資料，其餘四張表（`projects`、`design_iterations`、`synthesis_results`、`rules_effectiveness`）為空，待使用者透過 Cursor 進行 HLS 設計後方有資料寫入。實際設計流程可參考 [demo 目錄](https://github.com/aicoforge/cursor-hls-kb/tree/main/demo)。

以下三張截圖取自已經進行多次設計的資料庫，提供知識庫實際內容的視覺概念。各欄位的詳細語意與使用方式請參閱本文後續的「系統功能」章節，或 [cursorwork 目錄](https://github.com/aicoforge/cursor-hls-kb/tree/main/cursorwork) 中的 `.mdc-template` 規則檔。
...
由 `import_rules.py` 匯入的規則總表，可看到 UG1399 官方規則與使用者自定義規則按 category（如 `dataflow`、`pipeline`）分類儲存。

**截圖 2：`design_iterations` — 程式碼快照**

<p align="center">
 <img src="https://github.com/user-attachments/assets/135660d5-b03d-44bb-b97c-20064e3038d4" width="80%">
</p>

每次 Cursor 設計迭代會寫入 `code_snapshot`，右側展開視窗為某次 matmul（矩陣乘法）迭代的程式碼 header comment，記錄合成結果與本次套用的規則。

**截圖 3：`design_iterations` — 推理與 pragma 紀錄**

<p align="center">
 <img src="https://github.com/user-attachments/assets/58835c98-84fb-4fae-aec2-4a729146a6e2" width="80%">
</p>

同一張表的另一視角，呈現本次套用的 pragma、使用者原始輸入、及 Cursor 的推理脈絡——這些設計過程資訊在一般 LLM 對話中會隨對話結束而消失，知識庫則將其結構化保存供日後檢視與重現。
