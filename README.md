...
**Step 4: Browse the Database Tables**

After connecting, you can browse the 5 tables in the database. Right after installation, only `hls_rules` contains data (imported by `import_rules.py`); the other four tables (`projects`, `design_iterations`, `synthesis_results`, `rules_effectiveness`) are empty and will be populated as users run HLS designs through Cursor. For the design workflow, refer to the [demo directory](https://github.com/aicoforge/cursor-hls-kb/tree/main/demo).

The following three screenshots, taken from a database with multiple completed designs, give a visual sense of what the knowledge base actually stores. For detailed field semantics and usage, see the **System Features** section later in this document, or the `.mdc-template` rule files in the [cursorwork directory](https://github.com/aicoforge/cursor-hls-kb/tree/main/cursorwork).

**Screenshot 1: `hls_rules` — Rule Library**

<p align="center">
 <img src="https://github.com/user-attachments/assets/d0c18731-622e-4486-9b6d-b1d29c8ceb4d" width="80%">
</p>

The unified rule table imported by `import_rules.py`, showing UG1399 official rules and user-defined rules organized by category (such as `dataflow` and `pipeline`).

**Screenshot 2: `design_iterations` — Code Snapshot**

<p align="center">
 <img src="https://github.com/user-attachments/assets/135660d5-b03d-44bb-b97c-20064e3038d4" width="80%">
</p>

The `code_snapshot` written by each Cursor design iteration. The pop-up on the right shows the header comment of a matmul (matrix multiplication) iteration, recording synthesis results and the rules applied.

**Screenshot 3: `design_iterations` — Reasoning and Pragma Record**

<p align="center">
 <img src="https://github.com/user-attachments/assets/58835c98-84fb-4fae-aec2-4a729146a6e2" width="80%">
</p>

Another view of the same table, showing the pragmas applied, the user's original input, and Cursor's reasoning trace — design-process information that would otherwise vanish at the end of a typical LLM conversation, but is preserved here in structured form for later inspection and reproduction.
